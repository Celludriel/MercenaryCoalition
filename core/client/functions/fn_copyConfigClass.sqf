	...
	OUTPUT:
		_partialClass: TYPE - STRING | USED BY THE FUNCTION TO CREATE OUTPUT, DO NOT OVERWRITE
			...
			ex.
				...
				_whatINeed = [configfile >> "CfgVehicles" >> "Land_Cargo20_military_green_F"] call _fnc_copyConfigClass;
	TO CLIPBOARD:
...
_params = [];
{
	...
	for "_i" from 0 to _numParams do
	{
		...
		{
			_output = [_newConfig, _numTabs + 1, _output] call MCCLN_fnc_copyConfigClass;
		} else
		{
			_newParam = _param;
			_data = nil;
